...
struct malloc_context ctx = { 0 };		// 导出符号是 __malloc_hook
...
static uint32_t try_avail(struct meta **pm)
{
	struct meta *m = *pm;
	uint32_t first;
	if (!m) return 0;		// 没有active
	uint32_t mask = m->avail_mask;		
	if (!mask) {			// 没有可用的块
		if (!m) return 0;	// 好像没用
		if (!m->freed_mask) {	// 如果全都没有free
			dequeue(pm, m);		// 清出去
			m = *pm;		// 下一个meta
			if (!m) return 0;
		} else {
		...
		if (mask == (2u<<m->last_idx)-1 && m->freeable) {	// 如果全部都free掉了，就跳过去
			m = m->next;
			*pm = m;
			mask = m->freed_mask;
		}
		...
		// 按照作者的说法，优先使用另一个group的free的slots也不会去激活一个没有完全激活的group，除非万不得已
...
static int alloc_slot(int sc, size_t req)
{
	...
	if (first) return a_ctz_32(first);		// a_ctz_32： 0b100 -> 0x2

	struct meta *g = alloc_group(sc, req);		// alloc 一个group
	if (!g) return -1;

	g->avail_mask--;							// -1 就是全都可用
	queue(&ctx.active[sc], g);					// 上链
	return 0;
}

void *malloc(size_t n)
{
	if (size_overflows(n)) return 0;		// check overflow
	struct meta *g;
	uint32_t mask, first;
	int sc;    // size_class
	int idx;
	...
	if (n >= MMAP_THRESHOLD) {				// 如果大于阈值，用mmap
	...
	rdlock();			// 互斥锁
	g = ctx.active[sc];	//得到一个meta,也可能是空
	...
	for (;;) {							
		...
		first = mask&-mask;   // 这种写法是获得从低位开始第一个1
		if (!first) break;	  // 就是mask = 0
		if (RDLOCK_IS_EXCLUSIVE || !MT)   // 1
			g->avail_mask = mask-first;  // 去掉当前找到的那个mark
		else if (a_cas(&g->avail_mask, mask, mask-first)!=mask)  // 不执行
			continue;
		idx = a_ctz_32(first); // 返回第一个1的位置
		goto success;
	}
	upgradelock();

	idx = alloc_slot(sc, n);		// 没有可用的meta，就alloc一个slot
	if (idx < 0) {
		unlock();
		return 0;
	}
	g = ctx.active[sc];				// 返回这个alloc的slot
